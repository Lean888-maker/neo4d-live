...
export default function CartaRamalanPage({ params }) {
  ...
  // Load dynamic FAQs generated by our nightly SEO bot
  let dynamicFaqs = [];
  try {
    const faqPath = path.join(process.cwd(), 'src/app/data/faq_schema.json');
    if (fs.existsSync(faqPath)) {
      dynamicFaqs = JSON.parse(fs.readFileSync(faqPath, 'utf8'));
    }
  } catch (err) {
    console.warn("Could not load dynamic FAQs:", err);
  }
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="mt-12 bg-indigo-50 p-8 rounded-2xl border border-indigo-100">
          ...
          <div className="space-y-6">
            ...
            {/* Dynamic SEO FAQs injected here */}
            {dynamicFaqs.map((faq, index) => (
              <div key={`dynamic-faq-${index}`}>
