...
export default async function DreamsPage({ params }) {
  ...
  return (
    <>
      ...
      <main className="min-h-screen text-slate-200 pb-16 relative z-10 bg-slate-950">
        {/* Header Section */}
        ...
        <div className="max-w-3xl mx-auto px-4 mt-8">
          <div className="bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 md:p-10 border border-white/10 shadow-md relative overflow-hidden">
            <div className="absolute top-4 right-4 bg-red-500/20 text-red-300 font-extrabold text-[10px] uppercase tracking-widest px-3 py-1 rounded-full border border-red-500/30">
              今日梦境解密 (Daily Update)
            </div>
            ...
            {errorMsg ? (
              <div className="py-12 text-center space-y-4">
                ...
                <h3 className="font-bold text-white">无法载入梦境数据</h3>
                <p className="text-xs text-slate-400 max-w-md mx-auto">{errorMsg}</p>
                <Link href="/" className="mt-4 inline-block px-6 py-2.5 bg-slate-800 text-white font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-slate-700 transition-colors">
                  返回首页
                </Link>
              </div>
            ) : (
              <article 
                className="prose prose-invert prose-slate max-w-none prose-headings:font-black prose-headings:text-white prose-p:text-slate-300 prose-p:leading-relaxed text-sm md:text-base space-y-6"
                dangerouslySetInnerHTML={{ __html: content }}
              />
            )}

            {/* Bottom Disclaimer */}
            <footer className="mt-12 pt-6 border-t border-white/5 text-[10px] text-slate-500 font-medium leading-relaxed text-center">
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            <Link href={`/${lang}/predictions`} className="p-4 rounded-2xl bg-slate-900/60 backdrop-blur-xl border border-white/10 hover:border-amber-500/30 text-center shadow-sm hover:shadow transition-all group">
              <span className="text-2xl block mb-1">📈</span>
              <span className="text-xs font-black text-slate-200 group-hover:text-amber-400">今日万字预测</span>
            </Link>
            <Link href={`/${lang}`} className="p-4 rounded-2xl bg-slate-900/60 backdrop-blur-xl border border-white/10 hover:border-amber-500/30 text-center shadow-sm hover:shadow transition-all group">
              <span className="text-2xl block mb-1">🏆</span>
              <span className="text-xs font-black text-slate-200 group-hover:text-amber-400">最新开彩结果</span>
            </Link>
          </div>
        </div>
      </main>
